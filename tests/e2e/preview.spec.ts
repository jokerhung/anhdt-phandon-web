...
test("prototype dùng chung SlipPreview và PDF A7/A4 đúng khổ", async ({ page }) => {
  ...
  await expect(page.getByLabel("Phiếu phân đơn thử nghiệm")).toContainText("Số lượng 0 vẫn phải hiển thị");
  ...
  expect(Math.abs(a7.getPage(0).getSize().width - mmToPoints(74))).toBeLessThan(1);
  expect(Math.abs(a7.getPage(0).getSize().height - mmToPoints(105))).toBeLessThan(1);
  ...
  expect(Math.abs(a4.getPage(0).getSize().width - mmToPoints(210))).toBeLessThan(1);
  expect(Math.abs(a4.getPage(0).getSize().height - mmToPoints(297))).toBeLessThan(1);
});
